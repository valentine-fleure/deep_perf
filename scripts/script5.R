...
On calcule la durée moyenne d un passage grace au temps de presence et au nombre
de passaeg et le nombre de seconde où l espèce n est pas présente. 
Pour chaque passage on fait calcule un temps de présence grace a une loi binomiale
et la duree moyenne 
...
path = "data/data.csv"
matrix_species = load_database(path)
save(matrix_species, file = "Simu_20231107/matrix_species.Rdata")
fps = c(1,2,5,10,30)
duplicate = 30
scores = c(0.8, 0.85, 0.9, 0.95, 0.99)
for (frame_rate in fps){
  for (recall in scores){
    for (det_acc in scores){
      for (identif_acc in scores){
        for (safe_predict in scores){
          print(Sys.time())
          ...
          file_name = paste0("Simu_20231107/", frame_rate,"fps_", recall, "R_", det_acc, "accD_", identif_acc, "accI_", safe_predict, "sp.Rdata")
...
# level_order = paste0("species_", 1:nbr_specires)
det_acc = 0.9
recall = 0.9
identif_acc = 0.9
safe_predict = 0.9
...
#   summary = matrix_recall |>
#     dplyr::mutate(frame = 1:nrow(matrix_recall)) |>
#     tidyr::pivot_longer(cols = 1:ncol(matrix_recall),
#                         names_to = "species",
#                         values_to = "nbr") |>
#     dplyr::filter(nbr != 0) |>
#     tidyr::uncount(nbr) |>
#     stringr::str_replace_all(species, "species_", "")
#   
#   
#   
# summary = summary |>
